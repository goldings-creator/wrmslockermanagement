...
export default function App() {
  ...
  useEffect(() => {
    ...
    return () => {
      unsubSettings(); unsubLockers(); unsubStudents(); unsubLogs();
    };
  }, [user]);
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans pb-20 print:bg-white print:pb-0">
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr className="bg-slate-100 border-b-2 border-slate-900 text-left text-slate-600">
              ...
              <th className="p-4 text-[10px] font-black uppercase tracking-widest">Wing</th>
      ...
      <header className="bg-white border-b sticky top-0 z-40 p-4 flex justify-between items-center shadow-sm print:hidden">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => { setImportType('lockers'); setImportModalOpen(true); }} className="p-2 text-slate-400 border border-slate-200 rounded-xl hover:bg-slate-50 transition-colors shadow-sm" title="Import Locker CSV"><FileUp size={20}/></button>
